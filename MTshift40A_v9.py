...
def shift40A(params):
	...
	for i in l1:
		...
		# now shift 80A, which is always necessary
		shx_v1 = shx + DIMER*math.cos(-psi/180.0*math.pi)
                shy_v1 = shy + DIMER*math.sin(-psi/180.0*math.pi)
                shx_v2 = shx - DIMER*math.cos(-psi/180.0*math.pi)
                shy_v2 = shy - DIMER*math.sin(-psi/180.0*math.pi)

                if max(abs(shx_v1),abs(shy_v1)) < max(abs(shx),abs(shy)):
                        shx = shx_v1
                        shy = shy_v1
                if max(abs(shx_v2),abs(shy_v2)) < max(abs(shx),abs(shy)):
                        shx = shx_v2
                        shy = shy_v2
